...
func main() {
	licenseKey := "7a997b47a356b861c27b727a1093104e55eace9eb185ccab3c8a463c3e1bb842"
	...
	db, err = sqlx.Connect("postgres", "user=postgres password=postgres dbname=postgres sslmode=disable host=postgres")
	if err != nil {
	...
	err = minioClient.MakeBucket(context.Background(), "pdfs", minio.MakeBucketOptions{Region: "us-east-1"})
	if err != nil {
		exists, errBucketExists := minioClient.BucketExists(context.Background(), "pdfs")
	...
	app.Get("/pdfs", listPDFs)
	app.Get("/pdfs/:id", getPDF)
	app.Get("/search", searchWord)
	app.Get("/pdfs/:id/page/:page", getPageAsImage)
	app.Delete("/pdfs/:id", deletePDF)
